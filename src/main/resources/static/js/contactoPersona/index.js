...
//var url = "http://localhost:8080";   // Esto esta definido en el layout footer
var contactoPersona = {}; 
...
function obtenerListaPrincipal()  // Obtener lista de ContactoPersona de la Web Api 
{ 
		...
		{ 
			...
			{ 
				...
				{ 
					...
					fila.insertCell(3).innerHTML = c.ultimaFechaEnvioCorreo; 
...
function construirObjeto()  // Colocar los valores del objeto de tipo ContactoPersona en el html 
{ 
	Array.from(document.getElementsByClassName("contactoPersona")).forEach(c => 
	{ 
		if (c.name != undefined) 
		{ 
			c.value = contactoPersona[c.name];  // Coloca los valores del objeto en cada elemento del form 
		} 
	}); 
} 

function subirExcel(evento)
{
...
function crearNuevo()  // Iniciar creación de nuevo ContactoPersona 
{ 
	contactoPersona = { 
		haRespondido: "", 
...
function guardar(evento)  // Guardar y Actualizar ContactoPersona en la Web Api 
{ 
	evento.preventDefault(); 
...
function validar(contactoPersona)  // Validar ContactoPersona 
{ 
...
function editar(evento, id)  // Obtener ContactoPersona de la Web Api por su id 
{ 
	evento.preventDefault(); 
...
function eliminar(id)  // Eliminar ContactoPersona por su id 
{ 
