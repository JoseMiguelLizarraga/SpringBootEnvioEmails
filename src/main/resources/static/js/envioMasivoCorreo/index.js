...
//var paginaActualContactoPersona = 0;    //Paginación para mostrar elementos de la entidad ContactoPersona    
//var totalPaginasContactoPersona = 0;    //Paginación para mostrar elementos de la entidad ContactoPersona
...
function inicializarSelectBuscador(evento)  // Convertir select normal en un select buscador con paginacion  
{ 
...
{ 
	...
	var usuarioId = document.getElementById("buscador_usuario_select2");   // Toma el valor del select2 en el buscador para buscar por Usuario 
...
function obtenerListaPrincipal()  // Obtener lista de EnvioMasivoCorreo de la Web Api 
{ 
...
function construirObjeto()  // Colocar los valores del objeto de tipo EnvioMasivoCorreo en el html 
{ 
	...
	{ 
		...
		if (envioMasivoCorreo.estado != "finalizado") 
		{
			...
			fila.insertCell(1).innerHTML = '<div>' + 
...
//				c.contactoPersona.ultimaFechaEnvioCorreo : ""; 
			...
			fila.insertCell(2).innerHTML = "<button type='button' onclick='quitarEnvioCorreoContactoPersona(" + index + ")' class='btn btn-danger'> Remover </button>"; 
		}
		else 
		{
			fila.insertCell(0).innerHTML = c.estado; 
			fila.insertCell(1).innerHTML = c.contactoPersona.email; 	
...
function crearNuevo()  // Iniciar creación de nuevo EnvioMasivoCorreo 
{ 
	envioMasivoCorreo = { 
		asuntoCorreo: "", 
		emailOrigenEnvio: localStorage.getItem("token_email_envio_cartas"), 
...
function preguntarEnviarEmail(evento)
{
...
function guardar(evento)  // Guardar y Actualizar EnvioMasivoCorreo en la Web Api 
{ 
...
function validar(envioMasivoCorreo)  // Validar EnvioMasivoCorreo 
{ 
...
function editar(evento, id)  // Obtener EnvioMasivoCorreo de la Web Api por su id 
{ 
	evento.preventDefault(); 
	...
	{ 
		...
		{ 
			...
			{ 
				...
				this.envioMasivoCorreo["listaEnvioCorreoContactoPersona"].forEach(c=> {  // Recorre los elementos de tipo EnvioCorreoContactoPersona que pertenecen a EnvioMasivoCorreo 
...
function eliminar(id)  // Eliminar EnvioMasivoCorreo por su id 
{ 
...
function agregarNuevoEnvioCorreoContactoPersona()  // Agregar a EnvioMasivoCorreo un detalle de tipo EnvioCorreoContactoPersona 
{ 
...
function actualizarEnvioCorreoContactoPersona(evento, envioCorreoContactoPersona)  // Actualizar detalle de tipo EnvioCorreoContactoPersona que pertenece a EnvioMasivoCorreo 
{ 
...
function quitarEnvioCorreoContactoPersona(indice)  // Quitar detalle de tipo EnvioCorreoContactoPersona que pertenece a EnvioMasivoCorreo 
{ 
